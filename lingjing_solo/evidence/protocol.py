"""Public evidence schema and recording replay boundary.

The boundary is intentionally dependency-light: JSON-compatible values only,
explicit required fields, and fail-closed validation for malformed recordings.
"""
...
class EvidenceValidationError(ValueError):
    """Raised when evidence cannot be safely validated or replayed."""
...
def build_manifest(*, run_id: str, game_id: str, branch: str, commit: str,
                   module_versions: dict[str, str], evidence_tier: str, mode: str,
                   game_specific: dict[str, Any] | None = None, seed: Any = None,
                   limits: dict[str, Any] | None = None, source_recording: str | None = None) -> dict[str, Any]:
...
def build_tick(*, run_id: str, episode_id: str, tick: int, frame: Any, state: str,
               levels_completed: int, legal_actions: list[str], state_hash: str,
...
def build_verification_report(*, run_id: str, tier: str, verdict: str,
                              ...
                              evidence_refs: list[str], limitations: list[str] | None = None,
                              game_specific: dict[str, Any] | None = None) -> dict[str, Any]:
...
def validate_manifest(value: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(value, ("schema_version", "run_id", "game_id", "agent_build", "project_branch",
                     "module_versions", "evidence_tier", "mode", "limits", "status", "artifacts",
                     "game_specific"), "manifest")
    if value["schema_version"] != SCHEMA_VERSION:
        raise EvidenceValidationError("unsupported schema_version")
    if not value["run_id"] or not value["game_id"] or not value["agent_build"] or not value["project_branch"]:
...
def validate_tick(value: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise EvidenceValidationError("tick must be an object")
    _require(value, ("schema_version", "run_id", "episode_id", "tick", "frame", "requested_action",
                     "settled_frame", "state", "levels_completed", "score", "legal_actions",
                     "state_hash", "evidence_refs", "game_specific"), "tick")
    if value["schema_version"] != SCHEMA_VERSION:
        raise EvidenceValidationError("unsupported schema_version")
...
def validate_verification_report(value: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(value, dict):
    ...
    if value["schema_version"] != SCHEMA_VERSION or value["verdict"] not in {"PASS", "FAIL", "BLOCKED"}:
...
def _frame_hash(frame: Any) -> str:
    encoded = json.dumps(frame, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()
...
def _action(value: Any) -> dict[str, Any]:
    ...
    for axis in ("x", "y"):
        if axis in payload and (not isinstance(payload[axis], int) or not 0 <= payload[axis] <= 63):
...
def replay_recording(path: str | Path, *, legal_actions: list[str]) -> ReplayResult:
    """Replay JSONL into auditable transitions without mutating the source file."""
    ...
    with Path(path).open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, 1):
            ...
            transitions.append({
                "before_frame": previous["frame"], "action": action, "after_frame": data["frame"],
                "before_hash": _frame_hash(previous["frame"]), "after_hash": _frame_hash(data["frame"]),
                "state": state, "levels_completed": levels_completed,
                "server_action_input": data.get("server_action_input"),
            })
            previous = data
    if previous is None:
        raise EvidenceValidationError("recording has no baseline")

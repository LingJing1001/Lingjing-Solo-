...
### 阻塞点 1：Level 1/2 已闭环，Level 3–7 尚待扩展
...
- Level 1 和 Level 2 已完成；下一步转入 Level 3–7 的玩家、目标、开关参数和路线反推。
...
- [x] 已提取 Level 2–7 官方 source 的目标参数、起点、目标坐标及交互开关坐标；Level 2 起点 `(29,40)`、目标 `(14,40)`、旋转开关 `(49,45)`、目标 rotation `270`。该条早期离线记录曾显示只完成 Level 1，已被后续真实远程 scorecard `a726619e-4217-405b-beca-81fb1eb849ab` supersede；Level 2 现已确认通过。

### 阻塞点 2：Level 3–7 的目标机制和路线尚未接入

- Level 1 的胜利条件已由远程 scorecard 验证；Level 2 也已通过真实远程路线，但 Level 3–7 的目标颜色、形状、旋转、调色和多目标机制仍未完成验证。
- 改进方向：按 Level 3–7 逐关读取官方 source，建立最小状态转移表和低于 baseline 的路线，再用真实 scorecard 验证，不把 Level 2 的路线假设泛化到后续关卡。

### 阻塞点 3：planner 有路线生成能力，但没有可执行的起点到目标路线

- `plan_waypoints()` 能生成 Manhattan 路线，但 Level 3–7 尚未确认可通行地标、碰撞规则和动作持续步数；不能把 Level 1/2 的静态路线直接泛化到后续关卡。
- 改进方向：逐关建立 source 参数和最小状态转移表，使用离线候选搜索后再提交真实 scorecard；失败立即停止并保存 recording。
...
- [ ] 暂不扩展 Level 3-7、LLM 规则归纳或大规模 scorecard 重试。
...
- **验收标准：** Level 1 至少保持实验基线水平，Level 2 已有真实远程通过证据；后续完成 Level 3–7。
...
- 历史实验报告中的 Level 2 失败结果已被后续 scorecard `a726619e-4217-405b-beca-81fb1eb849ab` supersede；Level 3+ 仍需新的可复现实验 fixture 和真实 scorecard。

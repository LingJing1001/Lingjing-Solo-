...
if (( ! CHECK_ONLY )); then
  ...
  # run_ar25_r234.py 被上面的 cp -R 带进了 agents/strategies/，但它 import 依赖 arc_adaptor/paths.py，
...
printf 'arc_commit=%s\n' "$(git rev-parse HEAD)"
printf 'synced_files:\n'
# agents/__init__.py **不在**同步清单里：规则 ② 要求不覆盖 ARC 原生 exports，
...
for rel in "${SOURCE_FILES[@]}"; do
  if [[ -d "$rel" ]]; then
    ...
    if [[ ! -f "$source_file" && "$rel" == tests/unit/* ]]; then
      # arc_adaptor 自带测试在 arc_adaptor/tests/，仓库级测试在 <root>/tests/。
      if [[ -f "$SCRIPT_DIR/tests/${rel##*/}" ]]; then
        source_file="$SCRIPT_DIR/tests/${rel##*/}"
      else
        source_file="$LINGJING_ROOT/tests/${rel##*/}"
      fi
    fi
    source_hash="$(sha256sum "$source_file" | cut -d' ' -f1)"
